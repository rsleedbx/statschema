"""
Shared implementation for db_stats_collector.

All internal helpers and per-column stats collection live here.
Dialect-specific collector modules expose thin wrappers over _collect_column_stats.
"""
...
from ..stats_model import ColumnStats, CompositeColumnStats, MostCommonValue, TableStats


# ---------------------------------------------------------------------------
# CollectionTiming and CollectionConfig (moved here from db_stats_collector)
# ---------------------------------------------------------------------------
...
class CollectionTiming:
    ...
    full_mcv_ms: float = 0.0
    ntile_hist_ms: float = 0.0
    samp_nd_ms: float = 0.0
    rank_corr_ms: float = 0.0
...
def _collect_column_stats(  # pragma: no cover
    ...
    # ── pred_cols short-circuit ─────────────────────────────────────────────
    # When pred_cols is active and this column is NOT a predicate column, skip
    # the expensive enrichment (avg_width, MCV, histogram).  min/max is always
    # collected above because the data generator needs value ranges even for
    # non-predicate columns: wrong ranges → wrong page density → wrong costs.
...
def _collect_pg_composite_stats(  # pragma: no cover
    ...
    """
    from ..stats_model import CompositeColumnStats, MCVCombination
