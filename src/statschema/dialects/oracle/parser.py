"""oracle DDL parser — thin wrapper over the shared sqlglot-based parser."""
...
def parse(sql: str) -> list[CanonicalTableSchema]:
    """Parse oracle DDL SQL into canonical table schemas."""
    return parse_ddl(sql, dialect=DIALECT)


def parse_file(path: str | Path) -> list[CanonicalTableSchema]:
    """Parse a oracle .sql file into canonical table schemas."""
