...
This guide shows how to run **SQL Server, MySQL, MariaDB, PostgreSQL, Oracle, NeonDB, CockroachDB, and IBM Db2** locally
...
| **IBM Db2 CE** | Lima VM + Podman + QEMU (x86_64) | x86_64 | **50000** | No ARM64 image; Python driver `ibm_db` has ARM64 wheels |
...
## IBM Db2 CE (Lima VM + Podman + QEMU, x86_64)

IBM Db2 Community Edition has no ARM64 Linux image.  Like Oracle, we run it inside a Lima
x86_64 VM.  Unlike Oracle, the Python driver `ibm_db` ships ARM64 macOS wheels so tests
run natively on Apple Silicon — only the server-side container needs QEMU.

Db2 is the most unique dialect in this project: sqlglot has no Db2 support, so DDL is
emitted by a hand-written emitter in `ddl_emitter.py`.  Parsing Db2 DDL input uses ANSI
SQL mode (empty sqlglot dialect).
...
First boot pulls the image (~2 GB) and runs Db2 setup.  **This takes 5–10 minutes.**
Watch progress:
...
```bash
.venv_test/bin/pip install ibm_db
# (already listed in requirements-test.txt after first run)
```

`ibm_db` bundles the IBM Data Server Driver for ODBC and CLI — no separate client installation needed.

### Stop / restart / delete

```bash
limactl stop  db2   # stops the VM (Db2 container persists via systemd)
...
|---|---|---|
| `CLOB` | `CLOB` | Canonical unbounded string; `TEXT` has no direct equivalent in Db2 |
| `BOOLEAN` | `BOOLEAN` | Native Db2 11.1+ type; `TRUE`/`FALSE` literals used in DDL defaults |
| `REAL` | `REAL` | 32-bit float; canonical "float" emits as `REAL` |
| `DOUBLE` | `DOUBLE` | 64-bit float; distinct from `FLOAT` in Db2 |
| `SMALLINT` | `INTEGER` | Canonical model widens SMALLINT → `integer` → emits as `INTEGER` |
| `CHAR(n)` | `VARCHAR` | Canonical model does not distinguish CHAR from VARCHAR; re-emits as `VARCHAR(n)` |
| `CHAR(36)` | `CHARACTER` | UUID stored as fixed CHAR since Db2 has no UUID type |
| `GENERATED ALWAYS AS IDENTITY` | `INTEGER` | Identity column; implicitly NOT NULL |
| No `VARBINARY` | `BLOB` | Fixed-size binary uses `BLOB(n)` in Db2 |

### Troubleshooting

**Container fails to start with "Operation not permitted"**
The `--privileged=true` flag is required for Db2.  Verify the Lima VM starts the container
with the correct flags by checking `limactl shell db2 -- podman ps`.

**"Setup has completed" never appears**
Db2 setup can take up to 10 minutes on the first boot due to database initialisation.
If the probe times out, delete the VM and try again: `limactl delete db2`.
