...
**Collect DDL and statistics from any live database, generate synthetic data, load — three commands:**
```bash
statschema collect  --dialect postgres --host db.example.com --user myuser --catalog prod
...
**`statschema collect` writes `schema.yaml` automatically — no manual authoring required. Edit distributions, weights, or null rates between runs to model different test scenarios:**

`orders_schema.yaml` *(auto-generated by `collect`; edit to tune)*:
```yaml
tables:
  - name: orders
...
`statschema collect` also writes `stats.yaml` automatically — no user interaction required. The file captures null rates, cardinality, MCVs, and histograms from the source. You can leave it as-is or edit any value before generating:

`stats.yaml` *(auto-generated by `collect`; edit to tune)*:
```yaml
version: "1"
...
tables:
  - name: orders
    ...
    columns:
      - name: order_id
        null_fraction: 0.0
        n_distinct: 1000000.0
        min_value: "1"
        max_value: "1000000"
        histogram_bounds: ["1", "250000", "500000", "750000", "1000000"]
      - name: status
        null_fraction: 0.0
        n_distinct: 5.0
        most_common_values:
          - {value: delivered,  frequency: 0.50}
          - {value: shipped,    frequency: 0.25}
          - {value: processing, frequency: 0.12}
          - {value: pending,    frequency: 0.08}
          - {value: cancelled,  frequency: 0.05}
      - name: ship_date
        null_fraction: 0.08        # ← edit to model more/fewer unshipped orders
        n_distinct: -0.9
        min_value: "2023-01-15"
        max_value: "2025-01-31"
```
